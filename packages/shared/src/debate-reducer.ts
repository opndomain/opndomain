import { z } from "zod";
import {
  TopicContextCurrentRoundConfigSchema,
  TopicContextVoteTargetSchema,
  PendingProvenanceContributionSchema,
  VoteKindSchema,
} from "./schemas.js";

export const MAX_CONTRIBUTION_BODY_LENGTH = 20000;

// --- Types derived from existing shared schemas ---

type TopicContextCurrentRoundConfig = z.infer<typeof TopicContextCurrentRoundConfigSchema>;
type TopicContextVoteTarget = z.infer<typeof TopicContextVoteTargetSchema>;
type PendingProvenanceContribution = z.infer<typeof PendingProvenanceContributionSchema>;

export type DebateStepInput = {
  beingId: string;
  topicId: string;
  body?: string;
...
export type TopicContext = {
  ...
  currentRoundConfig?: TopicContextCurrentRoundConfig | null;
  transcript?: Array<{
    id: string;
    roundId: string;
    beingId: string;
  ...
  voteTargets?: TopicContextVoteTarget[];
  pendingProvenanceContributions?: PendingProvenanceContribution[];
...
export type DebateResult = {
  ...
  nextAction:
    ...
    | { type: "generate_votes"; payload: { system: string; voteTargets: TopicContextVoteTarget[]; obligation: NonNullable<TopicContext["votingObligation"]>; deadlineIso: string | null } }
...
export function waitPayload(untilIso: string | null | undefined, reason: string) {
  return {
...
export function personaSystem(personaText: string | null | undefined, roundInstruction: TopicContext["currentRoundConfig"] extends infer T ? any : never) {
...
export function formatTranscript(transcript: TopicContext["transcript"] | undefined) {
...
export function buildBodyPrompt(context: TopicContext, input: DebateStepInput, personaText: string | null, roundInstruction: NonNullable<NonNullable<TopicContext["currentRoundConfig"]>["roundInstruction"]>) {
  return {
...
export function buildVotePrompt(context: TopicContext, personaText: string | null, roundInstruction: NonNullable<NonNullable<TopicContext["currentRoundConfig"]>["roundInstruction"]>) {
  return {
    system: [
...
export function validateBody(body: string | undefined) {
  if (body === undefined) {
    return null;
  }
...
export function validateVotes(context: TopicContext, votes: DebateStepInput["votes"]) {
  if (!votes) {
    return null;
  }
...
export function latestOwnScoredContribution(context: TopicContext, beingId: string) {
...
export function pendingProvenanceForVoteTargetRound(context: TopicContext) {
