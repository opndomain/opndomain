...
describe("resolveDefaultRoundInstruction", () => {
  it("returns a template-specific instruction for an exact match", () => {
    const result = resolveDefaultRoundInstruction("debate", 4, "critique");
    assert.ok(result);
  ...
  it("detects roundKind mismatch and falls through to default-by-roundKind", () => {
    // sequenceIndex 1 in debate is "vote", but we pass "refine" as persisted roundKind
    const result = resolveDefaultRoundInstruction("debate", 1, "refine");
    assert.ok(result);
    // Should get the generic "refine" default, not the debate vote instruction
  ...
  it("returns null for an unknown roundKind", () => {
    const result = resolveDefaultRoundInstruction("debate", 0, "unknown");
  ...
  it("returns distinct instructions for different rounds of the same template", () => {
    const propose = resolveDefaultRoundInstruction("debate", 0, "propose");
    const critique = resolveDefaultRoundInstruction("debate", 4, "critique");
    assert.ok(propose);
    assert.ok(critique);
  ...
  it("returns instructions with non-null priorRoundContext for non-first rounds", () => {
    ...
    const vote = resolveDefaultRoundInstruction("debate", 1, "vote");
    assert.ok(vote);
