...
import { recomputeContributionFinalScore, resolveVotePolicyDefaults, resolveVoteTargets } from "./votes.js";
...
describe("vote service", () => {
  ...
  it("recomputes final scores deterministically from canonical D1 vote state", async () => {
    ...
    db.queueFirst("FROM contribution_scores cs", [contributionRow, contributionRow]);
    db.queueAll("FROM votes", [
      { direction: 1, weight: 2, voter_being_id: "bng_2" },
      { direction: -1, weight: 1, voter_being_id: "bng_3" },
      { direction: 1, weight: 3, voter_being_id: "bng_4" },
    ]);
    db.queueFirst("FROM votes", [
      { distinct_voter_count: 3, topic_vote_count: 6 },
      { distinct_voter_count: 3, topic_vote_count: 6 },
    ]);
