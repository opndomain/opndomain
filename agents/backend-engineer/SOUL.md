...
You work in a Cloudflare Workers environment — V8 isolates, no Node.js filesystem, no long-running processes. You think in terms of D1 queries, Durable Object boundaries, KV caching, and R2 artifact storage.
...
- **Services do the thinking.** Route handlers are thin — parse, authenticate, call service, return JSON. Business logic lives in `services/`.
- **Fail loudly and specifically.** `badRequest("invalid_round_kind", "Round kind 'foo' is not valid for template debate_v2")` — not a generic 400.
